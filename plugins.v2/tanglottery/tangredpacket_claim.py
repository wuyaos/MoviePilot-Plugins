...
使用说明
========
1. 配置 Cookie(二选一,优先级: --cookie > 环境变量):
   - 环境变量(推荐,避免凭证进 shell 历史/日志):
       export TANGPT_COOKIE='c_secure_pass=xxxx'
     可含多个键值对,以分号分隔:
       export TANGPT_COOKIE='c_secure_pass=xxxx; other_key=yyyy'
   - 命令行参数(覆盖环境变量):
       python tangredpacket_claim.py --cookie 'c_secure_pass=xxxx'
   Cookie 从浏览器扩展导出(HttpOnly,JS 抓不到),必须含 c_secure_pass。

2. 常用运行方式:
   - 仅查看(dry-run,不实际领取):
       python tangredpacket_claim.py
   - 实际领取(单次):
       python tangredpacket_claim.py --claim --once
   - 持续轮询领取(每 60s 一轮,限流感知自适应):
       python tangredpacket_claim.py --claim --loop --interval 60
   - 快速模式(跳过 detail 直接 claim,平均分配红包安全,请求数减半):
       python tangredpacket_claim.py --claim --once --fast

3. 输出文件:
   - 事件记录 jsonl(默认 /tmp/tangredpacket_claim.log.jsonl,--out 指定):
     每行一个事件(claim_ok/claim_fail/claim_skip),供数据面板消费。
   - 统计汇总 json(默认 /tmp/tangredpacket_claim.summary.json,--summary 指定):
     by_sender/by_title/by_date/recent 聚合,每次成功领取后重算。

4. 限流与防封:
   - 解析响应头 x-ratelimit-remaining,接近 0 自动延长 sleep。
   - claim 后默认 sleep 1s,串行不并发,Cookie 401/403 自动熔断退出。

5. 退出码: 0 正常结束, 1 Cookie 失效/连接失败, 2 未配置 Cookie。
"""
...
def main() -> int:
    ...
    if not args.cookie or "c_secure_pass=" not in args.cookie:
        LOG.error("未配置 Cookie，请设置环境变量 TANGPT_COOKIE 或传入 --cookie")
        return 2
