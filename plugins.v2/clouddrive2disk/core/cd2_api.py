...
class Cd2Api:
    ...
    def _set_active_metadata(self, index: int):
        ...
        logger.info(f"【CloudDrive2Disk】已切换鉴权头格式 variant={variant}, token_fp={self._token_fingerprint}")
    ...
    def _init_token_root(self):
        try:
            ...
            if not info_token and not info_name:
                logger.warning(
                    "【CloudDrive2Disk】GetApiTokenInfo 未返回有效 token 信息，可能不是当前实例创建的 token"
                )
            ...
            if permissions and hasattr(permissions, "allow_list") and not permissions.allow_list:
                logger.warning("【CloudDrive2Disk】当前 API key 未授予目录读取权限 allow_list，插件将无法浏览文件")
            ...
            if e.code() == grpc.StatusCode.UNAUTHENTICATED:
                ...
                logger.warning(
                    "【CloudDrive2Disk】无法通过 GetApiTokenInfo 校验当前 token，请确认该 key 在当前实例有效"
                )
                return
            if e.code() == grpc.StatusCode.UNIMPLEMENTED:
                self._token_info_state = "unimplemented"
                logger.debug("【CloudDrive2Disk】当前 CloudDrive2 版本未提供 GetApiTokenInfo，空间统计将回退到根路径")
                return
            self._token_info_state = "error"
            logger.debug(f"【CloudDrive2Disk】读取 API key rootDir 失败，回退到 '/': {e}")
    ...
    def _preflight_authorized_access(self):
        try:
            ...
            logger.debug(f"【CloudDrive2Disk】账户状态预检跳过: {self._rpc_error_text(e)}")
        ...
        try:
            ...
            logger.debug(f"【CloudDrive2Disk】初始化预检跳过: {self._rpc_error_text(e)}")
    ...
    def _log_auth_error_once(self, action: str, target: str, error: grpc.RpcError):
        ...
        logger.debug(f"【CloudDrive2Disk】{action}失败: {target}, {self._rpc_error_text(error)}")
    ...
    def list(self, fileitem: FileItem) -> List[FileItem]:
        ...
        try:
            ...
            if e.code() == grpc.StatusCode.UNAUTHENTICATED:
                ...
                logger.error(f"【CloudDrive2Disk】浏览目录失败: {path}, {self._rpc_error_text(e)}")
            return []
        except Exception as e:
            logger.error(f"【CloudDrive2Disk】浏览目录失败: {path}, {e}")
            return []
    ...
    def iter_files(self, fileitem: FileItem) -> Optional[List[FileItem]]:
        ...
        try:
            ...
            if e.code() == grpc.StatusCode.UNAUTHENTICATED:
                ...
                logger.error(f"【CloudDrive2Disk】递归遍历目录失败: {root}, {self._rpc_error_text(e)}")
            return None
        except Exception as e:
            logger.error(f"【CloudDrive2Disk】递归遍历目录失败: {root}, {e}")
            return None
    ...
    def create_folder(self, fileitem: FileItem, name: str) -> Optional[FileItem]:
        ...
        try:
            ...
            if hasattr(resp, "result") and not self._is_success(resp.result):
                ...
                logger.error(f"【CloudDrive2Disk】创建目录失败: {target_path}, {error_msg}")
                return None
            ...
            logger.error(f"【CloudDrive2Disk】创建目录失败: {target_path}, {e}")
            return None
    ...
    def delete(self, fileitem: FileItem) -> bool:
        ...
        try:
            ...
            if not self._is_success(resp):
                logger.error(f"【CloudDrive2Disk】删除失败: {path}, {getattr(resp, 'errorMessage', '')}")
                return False
            return True
        except Exception as e:
    ...
    def rename(self, fileitem: FileItem, name: str) -> bool:
        ...
        try:
            ...
            if not self._is_success(resp):
                logger.error(
                    f"【CloudDrive2Disk】重命名失败: {src_path} -> {name}, {getattr(resp, 'errorMessage', '')}"
                )
                return False
            return True
        except Exception as e:
            logger.error(f"【CloudDrive2Disk】重命名失败: {src_path} -> {name}, {e}")
            return False
    ...
    def move(self, fileitem: FileItem, path: Path, new_name: str) -> bool:
        ...
        try:
            ...
            if not self._is_success(resp):
                logger.error(
                    f"【CloudDrive2Disk】移动失败: {src_path} -> {dst_dir}, {getattr(resp, 'errorMessage', '')}"
                )
                return False
            ...
            if target_name != src_name:
                ...
                if not self._is_success(rename_resp):
                    logger.error(
                        f"【CloudDrive2Disk】移动后重命名失败: {moved_path} -> {target_name}, "
                        f"{getattr(rename_resp, 'errorMessage', '')}"
                    )
                    return False
            return True
        except Exception as e:
            logger.error(f"【CloudDrive2Disk】移动失败: {src_path} -> {dst_dir}, {e}")
            return False
    ...
    def copy(self, fileitem: FileItem, path: Path, new_name: str) -> bool:
        ...
        try:
            ...
            if not self._is_success(resp):
                logger.error(
                    f"【CloudDrive2Disk】复制失败: {src_path} -> {dst_dir}, {getattr(resp, 'errorMessage', '')}"
                )
                return False
            ...
            if target_name != src_name:
                ...
                if not self._is_success(rename_resp):
                    logger.error(
                        f"【CloudDrive2Disk】复制后重命名失败: {copied_path} -> {target_name}, "
                        f"{getattr(rename_resp, 'errorMessage', '')}"
                    )
                    return False
            return True
        except Exception as e:
            logger.error(f"【CloudDrive2Disk】复制失败: {src_path} -> {dst_dir}, {e}")
            return False
    ...
    def download(self, fileitem: FileItem, path: Optional[Path] = None) -> Optional[Path]:
        ...
        try:
            ...
            logger.error(f"【CloudDrive2Disk】下载失败: {remote_path} -> {local_path}, {e}")
    ...
    def upload(
        ...
        try:
            try:
                ...
                logger.error(f"【CloudDrive2Disk】上传失败 [CreateFile]: {remote_path}, {e}")
                return None

            file_handle = int(getattr(create_resp, "fileHandle", 0) or 0)
            if file_handle <= 0:
                logger.error(f"【CloudDrive2Disk】上传失败，创建远端文件句柄失败: {remote_path}")
                return None
            ...
            with open(local_path, "rb") as f:
                while True:
                    ...
                    try:
                        ...
                        logger.error(
                            f"【CloudDrive2Disk】上传失败 [WriteToFile]: {remote_path}, "
                    ...
                    if bytes_written != len(data):
                        logger.error(
                            f"【CloudDrive2Disk】上传失败，写入长度不匹配: {remote_path}, "
            ...
            try:
                ...
                logger.error(f"【CloudDrive2Disk】上传失败 [CloseFile]: {remote_path}, {e}")
                return None
            if not self._is_success(close_resp):
                logger.error(f"【CloudDrive2Disk】上传失败，关闭文件失败: {remote_path}")
                return None
            file_handle = 0
            ...
            logger.error(f"【CloudDrive2Disk】上传失败: {local_path} -> {remote_path}, {e}")
            return None
        finally:
            if file_handle > 0:
                try:
    ...
    def _wait_upload_complete(
                    ...
                    if dest == normalized or dest.rstrip("/") == normalized.rstrip("/"):
                        ...
                        if status_enum in terminal_statuses:
                            if status_enum == CloudDrive_pb2.UploadFileInfo.Finish:
                                ...
                                logger.warning(
                                    f"【CloudDrive2Disk】云端上传终态非成功: {remote_path}, "
                        ...
                        if stall_elapsed >= stall_timeout:
                            logger.warning(
                                f"【CloudDrive2Disk】云端上传停滞超时({stall_timeout}s 无新进度): {remote_path}, "
                        ...
                        logger.debug(
                            f"【CloudDrive2Disk】等待云端上传: {remote_path}, status={status_str}, "
        ...
        elapsed = timeout
        logger.warning(f"【CloudDrive2Disk】等待云端上传超时({elapsed}s): {remote_path}")
        return False
    ...
    def usage(self) -> Optional[StorageUsage]:
        ...
        logger.info(f"【CloudDrive2Disk】开始空间统计, base_root={base_root}")
        ...
        try:
            ...
            logger.info(f"【CloudDrive2Disk】根目录下共 {len(roots)} 个子条目")
            for one in roots:
                ...
                if not is_cloud_root:
                    logger.debug(
                        f"【CloudDrive2Disk】跳过非云盘根目录: name={name}, path={full_path}, "
                ...
                try:
                    ...
                    logger.info(
                        f"【CloudDrive2Disk】云盘空间: name={name}, cloudName={cloud_name}, path={normalized}, "
                    ...
                    if t > 0 or u > 0 or f > 0:
                        ...
                        logger.debug(f"【CloudDrive2Disk】跳过空间全为零的云盘: {normalized}")
                except grpc.RpcError as e:
                    code = e.code()
                    if code == grpc.StatusCode.UNAUTHENTICATED:
                        ...
                        logger.debug(f"【CloudDrive2Disk】跳过无权限或无效路径的空间统计: {normalized}, {code}")
                    else:
                        logger.warning(f"【CloudDrive2Disk】获取空间信息失败: {normalized}, {e}")
                except Exception as e:
                    logger.warning(f"【CloudDrive2Disk】获取空间信息失败: {normalized}, {e}")
        except Exception as e:
            logger.debug(f"【CloudDrive2Disk】枚举空间统计路径失败: {base_root}, {e}")

        if not entries:
            # 没有 isCloudRoot 的子目录时回退到根路径
            logger.info("【CloudDrive2Disk】未找到 isCloudRoot 子目录，回退到根路径获取空间")
            try:
                ...
                logger.info(
                    f"【CloudDrive2Disk】根路径空间: total={self._human_size(total)}, "
                ...
                if total > 0 or used > 0 or free > 0:
                    ...
                    logger.info(
                        f"【CloudDrive2Disk】空间统计结果(根路径回退): total={self._human_size(total)}, "
                        f"available={self._human_size(available)}"
                    )
                    return StorageUsage(total=total, available=available)
            except Exception as e:
                logger.warning(f"【CloudDrive2Disk】根路径空间统计也失败: {e}")
            return None
        ...
        for t, u, f, path in entries:
            ...
            if key not in seen:
                ...
                logger.debug(
                    f"【CloudDrive2Disk】去重跳过: path={path}, total={self._human_size(t)}, "
        ...
        if total <= 0 and used <= 0 and free <= 0:
            logger.warning("【CloudDrive2Disk】所有云盘空间信息均为零")
            return None

        available = free if free > 0 else max(total - used, 0)
        logger.info(
            f"【CloudDrive2Disk】空间统计结果: {len(unique)} 个云盘, "
