...
- `core/runner.py`：单轮线性执行流程，支持不添加 qB 的一次性试运行。
- `core/page.py`：四张总览卡片和运行日志表；种子列完整换行显示长标题。
...
- “试运行一次”是独立一次性开关：执行同样的槽位、Freeleech、时间和体积复核，但不下载 `.torrent`、不调用 qB 添加，运行后自动关闭。
...
- 日志列：时间、状态、种子、推送、详情；种子名称完整换行显示，最多保留最近 20 条。
