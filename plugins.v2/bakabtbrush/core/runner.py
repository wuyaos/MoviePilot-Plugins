...
from .filtering import matches_final_filters, matches_size, sort_key
...
def _run_rss_discovery(
    ...
    downloading_before = current_downloading
    try:
        refreshed = _sync_qb_state(config, state, qb_instance)
        current_downloading = downloading_count(refreshed)
    except DownloaderError:
        current_downloading = downloading_before
    # qB 添加接口可能先应答、后可见；槽位按发送前占用 + 本轮推送数兜底，避免瞬时少报。
    current_downloading = max(current_downloading, downloading_before + len(added))
