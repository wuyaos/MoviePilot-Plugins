...
class ColorHelper:
    ...
    def adjust_color_macaron(color: Tuple[int, int, int]) -> Tuple[int, int, int]:
        """调整颜色为马卡龙风格（参数源自 a39908646/MediaCoverGenerator）"""
        h, s, v = ColorHelper.rgb_to_hsv(color)

        # 马卡龙：饱和度 20%-70%、亮度 55%-85%（更柔和、避免过亮）
        s = min(max(s, 0.2), 0.7)
        v = min(max(v, 0.55), 0.85)

        return ColorHelper.hsv_to_rgb(h, s, v)

    @staticmethod
    def find_dominant_vibrant_colors(image: "Image.Image", num_colors: int = 5) -> List[Tuple[int, int, int]]:
        """
        提取图像主要非黑白灰颜色 + 调整到马卡龙色系，色相相似度过滤（15° 阈值去重）。
        ...
        filtered = [p for p in pixels if ColorHelper.is_not_black_white_gray_near(p)]
        if not filtered:
            return []

        counter = Counter(filtered)
        candidates = counter.most_common(num_colors * 3)

        result: List[Tuple[int, int, int]] = []
        seen_hues: set = set()
        for color, _ in candidates:
            h, s, v = ColorHelper.rgb_to_hsv(color)
            adj_rgb = ColorHelper.adjust_color_macaron(color)
            hue_deg = int(h * 360)
            if any(abs(hue_deg - seen) < 15 for seen in seen_hues):
                continue
            if adj_rgb in result:
                continue
            result.append(adj_rgb)
            seen_hues.add(hue_deg)
