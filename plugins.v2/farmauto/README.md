...
MoviePilot V2 多站点农场自动化插件。智能交易与自动收获双模式，支持价格趋势、盈利区间、仓库分页、临期出售、思齐验证码/偷菜/点赞等能力，并提供 Vue 联邦工作台。

## 功能

- 多站点统一调度（PlayLet / NovaHD / 好学 / 拾刻 / 包子 / 思齐）
- 智能交易（smart）：仅在盈利区间 `[min, max]` 内出售，避免亏本与高价囤货
- 自动收获（harvest）：一键收获成熟作物并补种，临期物品强制出售
- 价格趋势滚动记录（每物品最近 20 采样）与图表展示
- 单站策略覆盖（JSON，按站覆盖 mode/proxy/利润率/dry_run）
- 拾刻批量出售（batch_sell）降低请求频次
- 思齐验证码收获（OCR 优先 + 逐格降级）、偷菜/点赞每日限额、扩地（默认全关）
...
| 模式 | 触发 | 收获 | 出售 |
|---|---|---|---|
| smart 智能交易 | 仅可盈利作物 | 仅可盈利才收 | 盈利区间内出售 |
| harvest 自动收获 | 所有成熟作物 | 全收+补种 | 临期优先+可盈利出售 |
