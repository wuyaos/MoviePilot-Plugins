...
## 迭代 5 反思
- 已完成：架构拆分、基础层测试、8 个重叠站点全部迁移为明确适配器；通用 NexusPHP 已从注册/配置页移除；重试锁冲突已修复；本地 MP 验证配置页/数据页可用。
- 进展良好：每批站点独立小文件，mock 测试 + checkpoint 节奏清晰；当前 31/31 测试通过；未执行真实副作用。
- 当前风险：审查指出的 retry_notify 无效、feedback_timeout 未接入、重试状态仅存内存（MP 重启丢失）仍未处理；独有站点迁移中需注意 ptautotask 旧实现用了 stateless CustomRequests，迁移后应走统一 session。
- 调整：独有站点多为标准 NexusPHP 任务申领/签到，无喊话反馈需求，可按相同模式批量迁移；先写适配器+mock，再 MP 加载验证。
- 下一优先级：迁移蟹黄堡、柠檬、躺平三个 ptautotask 独有站点，补 mock 测试，加载器/表单回归验证。

## 迭代 5 进度（阶段 E 第一批）
- 已新增蟹黄堡适配器（3 任务）：签到、保种魔王任务申领、力争全勤任务申领；任务申领走统一 session 并用 parse_json_response 解析。
- 已新增柠檬适配器（2 任务）：签到（特殊页面解析）、每日神游（免费抽奖，POST type=0）。
- 已新增躺平适配器（2 任务）：签到、BUG/VIP 任务申领（双 ID）。
- 已新增 11 个 mock 测试覆盖三个站点的匹配、JSON 任务申领成功/失败/多 ID、抽奖成功/空响应、任务元数据。
- 验证：42/42 mock 测试通过；全部 Python AST 解析通过；`git diff --check` 通过。
- 本地 MoviePilot 强制重载成功，日志加载 11 个明确站点（藏宝阁 3 / 13City 5 / 蟹黄堡 3 / 柠檬 2 / LongPT 4 / PTLGS 2 / Ptskit 3 / 青蛙 3 / 躺平 2 / 象站 3 / 织梦 2）；配置页和数据页接口均返回 200。
- 未执行真实签到、任务申领或抽奖；全部请求测试使用 mock。

## 迭代 6 进度（阶段 E 第二批）
- 已新增财神/CARPT 适配器（2 任务）：签到、天天快乐任务申领（id=5）；按 domain 匹配。
- 已新增财神(Cspt) 适配器（2 任务）：签到、西财神任务申领（id=5）；按 domain 或站点名匹配。
- 已新增大青虫(Cyanbug) 适配器（2 任务）：签到、喊话（青虫娘三连）；仅按 domain 匹配，避免与 13City（同为“大青虫”中文名）冲突。
- 已新增 9 个 mock 测试覆盖三个站点的匹配（含防误匹配）、JSON 任务申领成功/失败、多消息喊话、任务元数据。
- 验证：51/51 mock 测试通过；全部 Python AST 解析通过；`git diff --check` 通过。
- 本地 MoviePilot 强制重载成功，日志加载 14 个明确站点（藏宝阁 3 / CARPT 2 / 13City 5 / 蟹黄堡 3 / 财神 2 / 大青虫 2 / 柠檬 2 / LongPT 4 / PTLGS 2 / Ptskit 3 / 青蛙 3 / 躺平 2 / 象站 3 / 织梦 2）；配置页和数据页接口均返回 200。
- 未执行真实签到、任务申领或喊话；全部请求测试使用 mock。

## 迭代 7 反思
- 已完成：阶段 A-D 完成，阶段 E 已迁移 6 个 ptautotask 独有站点（蟹黄堡/柠檬/躺平/CARPT/财神/大青虫）；当前 51/51 测试通过，14 个明确站点本地 MP 加载正常。
- 进展良好：独有站点批量迁移节奏稳定，每批适配器+mock+MP 加载验证；未执行真实副作用。
- 当前风险：retry_notify 无效、feedback_timeout 未接入、重试状态仅存内存仍未处理；vclib 站点含复杂任务状态检查+魔力兑换逻辑，需单独一轮谨慎迁移；审查遗留问题需在阶段 H 前修复。
- 调整：本轮迁移三个简单标准 NexusPHP 站点（自由农场/垃圾堆/NovaHD），vclib 延后单独处理；继续先适配器+mock+MP 加载验证节奏。
- 下一优先级：迁移自由农场、垃圾堆、NovaHD；随后单独迁移 vclib；再进入 groupchatzone 独有站点。

## 迭代 7 进度（阶段 E 第三批）
- 已新增自由农场适配器（2 任务）：签到、每周做种任务申领（id=12）；domain 为 0ff.cc。
- 已新增垃圾堆适配器（2 任务）：签到、每月保种计划任务申领（id=1）。
- 已新增 NovaHD 适配器（2 任务）：签到、保种任务申领（id=3）。
- 已新增 9 个 mock 测试覆盖三个站点的匹配、JSON 任务申领成功/失败、任务元数据。
- 验证：60/60 mock 测试通过；全部 Python AST 解析通过；`git diff --check` 通过。
